...
class EmailService {
    ...
    private createTableRow(product: IProductDetail) {
        const { name, timestamp } = product;
        return `<tr><td>${name}</td><td>${new Date(timestamp)}</td></tr>`;
    }
    ...
    async sendEmail(userEmail: string, filePaths: string[], products: IProductDetail[]) {
        ...
        html += '<tr style="background-color: #f2f2f2;"><th>Product Name</th><th>Product Created At</th></tr>';
        let count = 0;
